...
genoSamps <- read.table("genoFiles/yriGeno_samples.txt")
...
files <- file.path("/lustre/alice3/scratch/vasccell/cs806/geuvadis/salmonQuant", exprSamps$V1, "quant.sf")
names(files) <- paste0(exprSamps$V1)
...
names(normExpr) <- exprSamps$V2[match(names(normExpr), exprSamps$V1)]
