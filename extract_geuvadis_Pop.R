...
sampIndex <- read.delim("geuvadis_Scripts/geuvadis.sequence.index", skip = 23)
allSamp <- sampIndex[sampIndex$ANALYSIS_GROUP == "mRNA", c(3, 10, 19)  ]
allSamp <- allSamp[-37, ]
allSamp2 <- data.frame(wget = "wget -nc", allSamp$PAIRED_FASTQ)

write.table(allSamp, "geuvadis_allSamp_mRNA_detail_extract.txt", row.names = F, quote = F)
write.table(allSamp2, "rawReads/geuvadis_allSamp_mRNA_download.sh", row.names = F, quote = F, col.names = F)
