...
allChr = do.call(rbind, lapply(c(1:22), function(x) 
  read.table(paste0("mappedReads/geuYriJunc/juncClus_perind.counts.gz.qqnorm_", x))))

# Get colnames from first line of chromosome 1
con <- file(paste0("mappedReads/geuYriJunc/juncClus_perind.counts.gz.qqnorm_1"),"r")
...
names(sd) <-  gsub("Aligned.out.sam.sorted.bam", "", names(sd))

# Rename to match genotype data
exprSamps <- read.table("exprFiles/matched_YRI_Pheno.txt")
names(sd) <- exprSamps$V2[match(names(sd), exprSamps$V1)]
...
# Prep intron PC
pc <- read.table("mappedReads/geuYriJunc/juncClus_perind.counts.gz.PCs", header = TRUE)
names(pc) <-  gsub("Aligned.out.sam.sorted.bam", "", names(pc))
names(pc) <- exprSamps$V2[match(names(pc), exprSamps$V1)]
pc <- pc[, !is.na(names(pc))]
covID <- data.frame(Covariates = paste0(rep("LCC_PC", 10), 1:10))
pc$id <- NULL
pc <- cbind(covID, pc)


# Export files for QTLtools ---------------------------------------------
sFileNamePrefix <- paste0("phenoFiles/geuYri_LCsplice_qtlTools_Ready")
sPhenoFileName <- paste0(sFileNamePrefix, ".bed")
sPhenoAnnotFileName <- paste0(sFileNamePrefix, "_IntronAnnotation.txt")
sPhenoPCFileName <- paste0(sFileNamePrefix, "_IntronPCs.txt")
...
# Export annotation and PC files
write.table(annot, file = sPhenoAnnotFileName, row.names = FALSE, sep = '\t')
write.table(pc, file = sPhenoPCFileName, row.names = FALSE, sep = '\t')
